(function () {
    ...
    function setVarTrue() {
        try {
            (window.GetPlayer && window.GetPlayer()).SetVar(VAR_NAME, true);
        } catch (e) {}
    }
    ...
    function fontsReady() {
        return (document.fonts && document.fonts.ready) ? document.fonts.ready.catch(() => {}) : Promise.resolve();
    }
    ...
    function safeDownload(dataUrl) {
        ...
        try {
            if (window.top && window.top !== window) {
                var a2 = window.top.document.createElement('a');
                a2.href = dataUrl;
                a2.download = FILENAME;
                window.top.document.body.appendChild(a2);
                a2.click();
                a2.remove();
                return true;
            }
    ...
    (async function run() {
        try {
            ...
            // To do this, we need to hide the other elements for the screenshot.
            ...
            // Hide the circles or any other image-like elements that are not text.
            // You may need to adjust these selectors depending on your HTML structure.
            var elementsToHide = clone.querySelectorAll('div[data-name^="circel"], img, svg, [id*="shape"]');
            ...
            // Wait for the text image to load before drawing
